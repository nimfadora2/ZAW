...
def gradient(colour):
	sobelx = cv2.Sobel(colour, cv2.CV_64F, 1, 0, ksize=5)
	sobely = cv2.Sobel(colour, cv2.CV_64F, 0, 1, ksize=5)
...
def HOG(I):
	grad, dir = max_gradient(I)
	hist = histogram(I, grad, dir)
	f = normalize(hist)
